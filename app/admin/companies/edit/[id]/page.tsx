...
import { X, Plus, ChevronLeft, ChevronRight, Trash2, Save, Loader2 } from "lucide-react"
...
export default function EditCompanyPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    experienceRequired: { min: 0, max: 5 },
    workModes: [],
  ...
  const fetchCompany = async () => {
    try {
      setLoading(true)
      const res = await apiService.companies.getById(companyId)
      const data = res.data
      ...
      setFormData({
        ...
        averagePackage: {
          min: data.averagePackage?.min || 0,
          max: data.averagePackage?.max || 0,
          currency: data.averagePackage?.currency || 'INR'
        },
        ...
        eligibilityCriteria: {
          minCGPA: data.eligibilityCriteria?.minCGPA?.toString() || '',
          minPercentage: data.eligibilityCriteria?.minPercentage?.toString() || '',
          educationLevel: data.eligibilityCriteria?.educationLevel || 'Any Graduate',
          allowedBranches: data.eligibilityCriteria?.allowedBranches || [''],
          maxBacklogs: data.eligibilityCriteria?.maxBacklogs?.toString() || '',
          yearOfPassing: data.eligibilityCriteria?.yearOfPassing?.map((y: any) => y.toString()) || [''],
          ageLimit: data.eligibilityCriteria?.ageLimit?.toString() || '',
        },

        hiringFreshers: data.hiringFreshers || false,
        experienceRequired: {
          min: data.experienceRequired?.min || 0,
          max: data.experienceRequired?.max || 5
        },
        workModes: data.workModes || [],
        remoteMinExperience: data.remoteMinExperience || 0,
      ...
      if (data.rolesData && data.rolesData.length > 0) {
        const mappedRoles = data.rolesData.map((role: any) => ({
          roleName: role.roleName,
          description: role.description || '',
          hiringPipeline: role.hiringPipeline || [],
          interviewQuestions: role.interviewQuestions || [],
          // Map problems to flat structure
          linkedDSAProblems: role.linkedDSAProblems?.map((link: any) => ({
            problem: link.problem?._id || link.problem,
            title: link.problem?.title || 'Unknown Problem',
            frequency: link.frequency || 'Medium',
            roundNumber: link.roundNumber,
            notes: link.notes || ''
          })) || [],
          linkedDevProblems: role.linkedDevProblems?.map((link: any) => ({
            problem: link.problem?._id || link.problem,
            title: link.problem?.title || 'Unknown Problem',
            frequency: link.frequency || 'Medium',
            roundNumber: link.roundNumber,
    ...
    try {
      ...
      const payload = {
        ...
        eligibilityCriteria: {
          ...
          minCGPA: formData.eligibilityCriteria.minCGPA ? Number(formData.eligibilityCriteria.minCGPA) : undefined,
          minPercentage: formData.eligibilityCriteria.minPercentage ? Number(formData.eligibilityCriteria.minPercentage) : undefined,
          maxBacklogs: formData.eligibilityCriteria.maxBacklogs ? Number(formData.eligibilityCriteria.maxBacklogs) : undefined,
          ageLimit: formData.eligibilityCriteria.ageLimit ? Number(formData.eligibilityCriteria.ageLimit) : undefined,
      ...
      await apiService.companies.update(companyId, payload)
